...
client = hvac.Client(url="https://vault.ecp.polaris.cvshealthcloud.com", token="s.hRtDiP0tkWvtu0HW8DnhGe0j", verify=False)
...
for item in list:
    ...
    if "sys" not in item and "cubbyhole" not in item and "request_id" not in item and "lease_id" not in item \
        ...
        os.system("rvault read %s -a https://vault.wcp.polaris.cvshealthcloud.com -t s.bIZ38NUPvGtVGiy3wv949rXK -f yaml | tail -n +2 > %s.yml" % (clean_key, clean_key))
